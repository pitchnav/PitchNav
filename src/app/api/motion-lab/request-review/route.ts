...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: analysis } = await admin.from('motion_analyses').select('id,title,created_at,user_id').eq('id', analysisId).single()
    ...
    const { data: profile } = await admin.from('profiles').select('full_name,email').eq('id', analysis.user_id).single()
    const reviewEmail =
